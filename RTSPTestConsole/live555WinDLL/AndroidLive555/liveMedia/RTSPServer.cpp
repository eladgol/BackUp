...
std::string to_string(T value)
{
	...
	os << value;
	const std::string ws(os.str().begin(), os.str().end());
	return ws;
}
#else
//#include <netinet/in.h>
...
std::string AddrToChar(sockaddr_in sa)
{
	char str[22/*INET_ADDRSTRLEN*/];
	inet_ntop(2/*AF_INET*/, &(sa.sin_addr), str, 22/*INET_ADDRSTRLEN*/);
	auto clientPort = sa.sin_port;
	std::string sOut = str;
//	sOut += " ";
	//sOut += to_string((unsigned int)clientPort);
	//std::string sOuts(sOut.begin(), sOut.end());
	return sOut;
}
...
Boolean RTSPServer::setUpTunnelingOverHTTP(Port httpPort) {
  ...
  envir() << "RTSPServer::setUpTunnelingOverHTTP httpPort " << httpPort.num() << "\n";
...
::unnoteTCPStreamingOnSocket(int socketNum, RTSPClientSession* clientSession, unsigned trackNum) {
  ...
  envir() << "RTSPServer::RTSPClientConnection::unnoteTCPStreamingOnSocket socket num " << socketNum << " session id " << sotcpHead->fSessionId << "\n";
...
void RTSPServer::stopTCPStreamingOnSocket(int socketNum) {
  ...
  streamingOverTCPRecord* sotcp
    = (streamingOverTCPRecord*)fTCPStreamingDatabase->Lookup((char const*)socketNum);
  if (sotcp != NULL) {
	  envir() << "RTSPServer::RTSPClientConnection::stopTCPStreamingOnSocket socket num " << socketNum << " session id " << sotcp->fSessionId << "\n";
...
void RTSPServer::RTSPClientConnection::handleCmd_OPTIONS() {
  ...
  envir() << "RTSPServer::RTSPClientConnection::handleCmd_OPTIONS Client" << ((const char*) AddrToChar(fClientAddr).c_str()) << "\n";
  envir() << "RTSPServer::RTSPClientConnection::handleCmd_OPTIONS " << fResponseBuffer << "\n";
}
...
::handleCmd_GET_PARAMETER(char const* /*fullRequestStr*/) {
  ...
  envir() << "RTSPServer::RTSPClientConnection::handleCmd_GET_PARAMETER Client" << (const char*)AddrToChar(fClientAddr).c_str() << "\n";
  envir() << "RTSPServer::RTSPClientConnection::handleCmd_GET_PARAMETER " << fResponseBuffer << "\n";
}
...
::handleCmd_SET_PARAMETER(char const* /*fullRequestStr*/) {
  ...
  envir() << "RTSPServer::RTSPClientConnection::handleCmd_SET_PARAMETER Client" << (const char*)AddrToChar(fClientAddr).c_str() << "\n";
  envir() << "RTSPServer::RTSPClientConnection::handleCmd_SET_PARAMETER " << fResponseBuffer << "\n";
}
...
::handleCmd_DESCRIBE(char const* urlPreSuffix, char const* urlSuffix, char const* fullRequestStr) {
  ...
  do {
	...
	envir() << "RTSPServer::RTSPClientConnection::handleCmd_DESCRIBE Client" << (const char*)AddrToChar(fClientAddr).c_str() << "\n";
	envir() << "RTSPServer::RTSPClientConnection::handleCmd_DESCRIBE " << fResponseBuffer << "\n";
  } while (0);
...
void RTSPServer::RTSPClientConnection::handleCmd_bad() {
  ...
  envir() << "RTSPServer::RTSPClientConnection::handleCmd_bad Client" << (const char*)AddrToChar(fClientAddr).c_str() << "\n";
  envir() << "RTSPServer::RTSPClientConnection::handleCmd_bad " << fResponseBuffer << "\n";
}
...
void RTSPServer::RTSPClientConnection::handleCmd_notSupported() {
  snprintf((char*)fResponseBuffer, sizeof fResponseBuffer,
	   ...
	   fCurrentCSeq, dateHeader(), fOurRTSPServer.allowedCommandNames());
  envir() << "RTSPServer::RTSPClientConnection::handleCmd_notSupported Client" << (const char*)AddrToChar(fClientAddr).c_str() << "\n";
  envir() << "RTSPServer::RTSPClientConnection::handleCmd_notSupported " << fResponseBuffer << "\n";
}

void RTSPServer::RTSPClientConnection::handleCmd_notFound() {
  setRTSPResponse("404 Stream Not Found");
  envir() << "RTSPServer::RTSPClientConnection::handleCmd_notFound Client" << (const char*)AddrToChar(fClientAddr).c_str() << "\n";
  envir() << "RTSPServer::RTSPClientConnection::handleCmd_notFound " << fResponseBuffer << "\n";
}

void RTSPServer::RTSPClientConnection::handleCmd_sessionNotFound() {
  setRTSPResponse("454 Session Not Found");
  envir() << "RTSPServer::RTSPClientConnection::handleCmd_sessionNotFound Client" << (const char*)AddrToChar(fClientAddr).c_str() << "\n";
  envir() << "RTSPServer::RTSPClientConnection::handleCmd_sessionNotFound " << fResponseBuffer << "\n";
}

void RTSPServer::RTSPClientConnection::handleCmd_unsupportedTransport() {
  setRTSPResponse("461 Unsupported Transport");
  envir() << "RTSPServer::RTSPClientConnection::handleCmd_unsupportedTransport Client" << (const char*)AddrToChar(fClientAddr).c_str() << "\n";
  envir() << "RTSPServer::RTSPClientConnection::handleCmd_unsupportedTransport " << fResponseBuffer << "\n";
}
...
void RTSPServer::RTSPClientConnection::handleHTTPCmd_notSupported() {
  snprintf((char*)fResponseBuffer, sizeof fResponseBuffer,
	   ...
	   dateHeader());
  envir() << "RTSPServer::RTSPClientConnection::handleHTTPCmd_notSupported Client" << (const char*)AddrToChar(fClientAddr).c_str() << "\n";
  envir() << "RTSPServer::RTSPClientConnection::handleHTTPCmd_notSupported " << fResponseBuffer << "\n";
}
...
void RTSPServer::RTSPClientConnection::handleHTTPCmd_notFound() {
  snprintf((char*)fResponseBuffer, sizeof fResponseBuffer,
	   ...
	   dateHeader());
  envir() << "RTSPServer::RTSPClientConnection::handleHTTPCmd_notFound Client" << (const char*)AddrToChar(fClientAddr).c_str() << "\n";
  envir() << "RTSPServer::RTSPClientConnection::handleHTTPCmd_notFound " << fResponseBuffer << "\n";
}
...
void RTSPServer::RTSPClientConnection::handleHTTPCmd_OPTIONS() {
  ...
  snprintf((char*)fResponseBuffer, sizeof fResponseBuffer,
	   ...
	   "\r\n",
	   dateHeader());
  envir() << "RTSPServer::RTSPClientConnection::handleHTTPCmd_OPTIONS Client" << (const char*)AddrToChar(fClientAddr).c_str() << "\n";
  envir() << "RTSPServer::RTSPClientConnection::handleHTTPCmd_OPTIONS " << fResponseBuffer << "\n";
...
void RTSPServer::RTSPClientConnection::handleHTTPCmd_TunnelingGET(char const* sessionCookie) {
  ...
  snprintf((char*)fResponseBuffer, sizeof fResponseBuffer,
	   ...
	   "\r\n",
	   dateHeader());
  envir() << "RTSPServer::RTSPClientConnection::handleHTTPCmd_TunnelingGET Client" << (const char*)AddrToChar(fClientAddr).c_str() << "\n";
  envir() << "RTSPServer::RTSPClientConnection::handleHTTPCmd_TunnelingGET " << fResponseBuffer << "\n";
...
#endif
    int bytesWritten = send(fClientOutputSocket, (char const*)fResponseBuffer, strlen((char*)fResponseBuffer), 0);
	envir() << "RTSPServer::RTSPClientSession::handleRequestBytes Client" << (const char*)AddrToChar(fClientAddr).c_str() << "\n";
	envir() << "RTSPServer::RTSPClientSession::handleRequestBytes " << fResponseBuffer << "\n";
    if (playAfterSetup) {
...
::authenticationOK(char const* cmdName, char const* urlSuffix, char const* fullRequestStr) {
  ...
  envir() << "RTSPServer::RTSPClientSession::parseAuthorizationHeader " << fResponseBuffer << "\n";
  return False;
}
...
::setRTSPResponse(char const* responseStr) {
  snprintf((char*)fResponseBuffer, sizeof fResponseBuffer,
	   ...
	   fCurrentCSeq,
	   dateHeader());
  envir() << "RTSPServer::RTSPClientSession::setRTSPResponse 1 " << fResponseBuffer << "\n";
...
::setRTSPResponse(char const* responseStr, u_int32_t sessionId) {
  snprintf((char*)fResponseBuffer, sizeof fResponseBuffer,
	   ...
	   dateHeader(),
	   sessionId);
  envir() << "RTSPServer::RTSPClientSession::setRTSPResponse s " << fResponseBuffer << "\n";
}
...
::handleCmd_SETUP(RTSPServer::RTSPClientConnection* ourClientConnection,
  ...
  envir() << "RTSPServer::RTSPClientSession::handleCmd_SETUP Start " << streamName << " trackID " << trackId << "\n";
//  envir() << "RTSPServer::RTSPClientSession::handleCmd_SETUP  Client " << (const char*)AddrToChar(ourClientConnection->fClientAddr).c_str() << "\n";
  do {
	...
	envir() << "RTSPServer::RTSPClientSession::handleCmd_SETUP streamingMode " << streamingMode << " Response " << ourClientConnection->fResponseBuffer << "\n";
	envir() << "RTSPServer::RTSPClientSession::handleCmd_SETUP Client" << (const char*)AddrToChar(ourClientConnection->fClientAddr).c_str() << "\n";
...
::handleCmd_withinSession(RTSPServer::RTSPClientConnection* ourClientConnection,
  ...
  ServerMediaSubsession* subsession;
  envir() << "RTSPServer::RTSPClientSession::handleCmd_withinSession  Client " << (const char*)AddrToChar(ourClientConnection->fClientAddr).c_str() << "\n";
...
::handleCmd_TEARDOWN(RTSPServer::RTSPClientConnection* ourClientConnection,
		     ServerMediaSubsession* subsession) {
  envir() << "RTSPServer::RTSPClientSession::handleCmd_TEARDOWN Start fNumStreamStates " << fNumStreamStates << "\n";
  unsigned i;
  for (i = 0; i < fNumStreamStates; ++i) {
	...
	|| subsession == fStreamStates[i].subsession) {
      if (fStreamStates[i].subsession != NULL) {
	fOurRTSPServer.unnoteTCPStreamingOnSocket(fStreamStates[i].tcpSocketNum, this, i);
	envir() << "RTSPServer::RTSPClientSession::handleCmd_TEARDOWN " << i << ":" << fNumStreamStates << " tcpSocketNum " << fStreamStates[i].tcpSocketNum << 
		" streamToken " << fStreamStates[i].streamToken << "\n";
	fStreamStates[i].subsession->deleteStream(fOurSessionId, fStreamStates[i].streamToken);
  ...
  envir() << "RTSPServer::RTSPClientSession::handleCmd_TEARDOWN  Client " << (const char*)AddrToChar(ourClientConnection->fClientAddr).c_str() << "\n";
...
::handleCmd_PLAY(RTSPServer::RTSPClientConnection* ourClientConnection,
  ...
  envir() << "RTSPServer::RTSPClientSession::handleCmd_Play Start rtspURL " << rtspURL << " size " << rtspURLSize << "\n";
  envir() << "RTSPServer::RTSPClientSession::handleCmd_Play  Client " << (const char*)AddrToChar(ourClientConnection->fClientAddr).c_str() << "\n";
  ...
  snprintf((char*)ourClientConnection->fResponseBuffer, sizeof ourClientConnection->fResponseBuffer,
	   ...
	   fOurSessionId,
	   rtpInfo);
  envir() << "RTSPServer::RTSPClientSession::handleCmd_Play Response " << ourClientConnection->fResponseBuffer << "\n";
  //envir() << "RTSPServer::RTSPClientSession::handleCmd_Play  Client " << (const char*)AddrToChar(ourClientConnection->fClientAddr).c_str() << "\n";
